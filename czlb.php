...
<html lang="zh-cn">
	...
	<p>
	战役简报：<br> 两方必须各有一个领队，各自的部队领队将在开战前一小时收到命令，依据命令配置你们的机动战士。<br>对战限定于最高20个回合，20回合后将判定胜负。<br>战斗爆发的地点在开战前一小时才会暴露。<br>机体装备不限 但是请考虑驾驶员。<br>武备方面，将对各类武器进行分级，依据其重量 体积 重要程度进行分级 限制武器搭载。<br>请在至少开战前两小时完成小队编组，那么 祝各位驾驶员好运！
	</p>
	</div>
	<div class="col-md-7">
	...
	function loadlog()
	{
		$.post("addlog.php",function(data){$('#logs').html(data);});
	}
	...
	function send()
	{
		...
		{   $('#adder').text('发送中......');
			...
			$.post("addlog.php",  { log:UE.getEditor('container').getContent() },function(data){
				$('#logs').html(data);
				$('#adder').text('发送完成');
	...
	肥细</div><div style='text-align:center'>
扎古</div><div style='text-align:center'>
本</div><div style='text-align:center'>
妞</div><div style='text-align:center'>
行剑</div><div style='text-align:center'>
guxiang</div><div style='text-align:center'>
6</div><div style='text-align:center'>
空航</div><div style='text-align:center'>
雪风</div>
